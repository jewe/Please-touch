...
public class TUIOzoneCollection implements PConstants {
	...
	void setZoneDataCore(int i, int xIn, int yIn, int wIn, int hIn) {
		...
		tzone[i].cx = tzone[i].y + tzone[i].h / 2;
	}
	...
	void modifyZone(int xIn, int yIn, int vxIn, int vyIn, int tpointIn, int zone) {
		...
		if (tzone[zone].primePoint == tpointIn && tzone[zone].draggable
			...
			setZoneData(zone, tzone[zone].xi
					+ (xIn - tpoint[tpointIn].coord[0][0]), tzone[zone].yi
					+ (yIn - tpoint[tpointIn].coord[0][1]), tzone[zone].w,
					tzone[zone].h);
		...
		// IF SCALABLE (two-finger gesture)
		if (touches.length == 2) {
			for (int i = 0; i < tpoint.length; i++) {
					...
					if (tpoint[i].zone0 == zone) {
						...
						if (tzone[zone].scalable) {
							...
							int midy = (int) (yIn * tpointInRatio + iLast[1]
									* iRatio);
							int newX = midx
									- (int) ((midx - tzone[zone].xi) * scl);
							int newY = midy
									- (int) ((midy - tzone[zone].yi) * scl);
